...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch(
    "../../netlify/functions/updateCarData/mockData/PopulateLocationData.json"
  )
  ...
  tryAccessDirectory(CarDataDirectoryUrl).then((isAccessible) => {
    if (isAccessible) {
      ...
      fetch(CarDataDirectoryUrl)
        ...
        .then((data) => {
          ...
          filteredData.forEach((carData) => {
            ...
            if (
              ["Good", "New"].includes(
                carData.inspectionReport.vehicleCondition
              )
            ) {
      ...
      tryAccessDirectory(CarDataFallbackRepoUrl).then((fallbackAccessible) => {
        if (fallbackAccessible) {
          ...
          console.log(`Local repo not accessible.`);
        }
      });
    }
  });
